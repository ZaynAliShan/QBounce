...
export default function VideoCategoryTabs() {
  ...
  const goTo = (cat, videoId = 0) => {
    navigate(`/train/${cat}/${videoId}`, { replace: true })
  }

  const active = category && CATEGORIES.includes(category) ? category : 'beginner'

  return (
    <div className="flex flex-wrap justify-center gap-2 sm:gap-3 md:gap-4 py-4 sm:py-5 md:py-6 px-1">
      {CATEGORIES.map((cat) => {
        ...
        return (
          <button
            key={cat}
            type="button"
            onClick={() => !locked && goTo(cat, 0)}
            disabled={locked}
            className={`
              capitalize border-2 min-w-0 min-w-[80px] sm:min-w-[88px] md:min-w-[100px] min-h-[44px] sm:h-11 rounded-lg font-semibold text-xs sm:text-sm md:text-base flex items-center justify-center gap-1.5 px-2 sm:px-3 overflow-hidden
